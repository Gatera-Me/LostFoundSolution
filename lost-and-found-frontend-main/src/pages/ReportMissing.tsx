...
export const ReportMissing = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:8080/lost-items', {
                ...formData,
                lostDate: new Date(formData.lostDate).toISOString(),
            }, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
            addNotification(`New lost item reported: ${formData.itemName}`);
            alert(`Missing item reported: ${formData.itemName}`);
            setFormData({ itemName: '', description: '', lostLocation: '', lostDate: new Date().toISOString().split('T')[0], status: 'OPEN' });
        } catch (error) {
        ...
        <div className="flex min-h-screen">
            <div className="w-64 bg-gray-200 p-6">
                ...
                <nav className="space-y-3">
                    ...
                    <Link to="/report-missing" className="block py-3 px-4 rounded-lg bg-gray-300 text-gray-800 font-semibold">Report Missing Items</Link>
            ...
            <div className="flex-1 bg-gray-100">
                ...
                <div className="p-10">
                    <div className="bg-white p-8 rounded-xl shadow-lg">
                        <h2 className="text-2xl font-bold mb-6 text-gray-800">Report a Missing Item</h2>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Location Last Seen</label>
                                <input
                                    type="text"
                                    name="lostLocation"
